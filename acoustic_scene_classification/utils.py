...
class MixupGenerator():
    ...
    def __init__(self, X_train, y_train, batch_size=32, alpha=0.2, shuffle=True, crop_length=400, y_train_2=None,
...
class SpecmixGenerator():
    ...
    def __data_generation(self, batch_ids):
        ...
        for i in range(X1.shape[0]) :
            mask, inv_mask = masking(X1[0, :, :, 0], ratio = 0.7,
...
class RandomMaskingGenerator():
    ...
    def __data_generation(self, batch_ids):
        ...
        lam = list()
        for i in range(X1.shape[0]) :
            mask, inv_mask = random_masking(X1[0, :, :, 0], ratio = np.random.rand(1)[0])
            lam.append(mask.sum() / (X.shape[1] * X.shape[2]))
            for j in range(X1.shape[3]) :
                X[i,:,:,j] = mask * X1[i,:,:,j] + inv_mask * X2[i,:,:,j]
        lam = np.array(lam)
...
class EnergymaskingGenerator():
    ...
    def __data_generation(self, batch_ids):
        ...
        lam = list()
        for i in range(X1.shape[0]) :
            mask, inv_mask = energy_masking2(X1[i, :, :, 0], X2[i, :, :, 0])
            lam.append(mask.sum() / (X.shape[1] * X.shape[2]))
            #lam.append((mask * X1[i, :, :, 0]).sum() / X[i, :, :, 0].sum())
...
def random_masking(spec, ratio=0.3) :
    mask = np.ones(spec.shape)

    x_lim = spec.shape[0]
    y_lim = spec.shape[1]
    num_pixel = x_lim * y_lim

    x = np.random.randint(0, x_lim, size=int(num_pixel * ratio))
    y = np.random.randint(0, y_lim, size=int(num_pixel * ratio))
...
def energy_masking(spec1, spec2) :
    mask = np.zeros(spec1.shape)
    inv_mask = np.ones(spec1.shape)
...
def energy_masking2(spec1, spec2) :
    mask = np.zeros(spec1.shape)
    inv_mask = np.ones(spec1.shape)
    ...
    if freq_only :
        spec_sum1 = np.sum(spec1, axis=1)
        spec_sum2 = np.sum(spec2, axis=1)

        for i in range(spec1.shape[0]) :
            if spec_sum1[i] >= spec_sum2[i] :
                mask[i, :] = 1
                inv_mask[i,:] = 0
            else :
                mask[i,:] = 0
        ...
        spec_sum1 = np.sum(spec1, axis=0)
        spec_sum2 = np.sum(spec2, axis=0)

        for j in range(spec1.shape[1]) :
            if spec_sum1[j] >= spec_sum2[j] :
                mask[:, j] = 1
                inv_mask[:, j] = 0
            else :
                mask[:, j] = 0

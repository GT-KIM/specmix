        ...
        for i, data in enumerate(TrainDataGen) :
            ...
            if idx % 1000 == 0 :
                ...
                if Overall_accuracy >= best_acc :
                    ...
                    state = {'model': model.state_dict(),
                             'optimizer': optimizer.state_dict()}
                    if not os.path.isdir("ckpt2_{}/".format(data_augment)):
                        os.makedirs("ckpt2_{}/".format(data_augment))
                    torch.save(state, "ckpt2_{}/model.pth".format(data_augment))

            if idx == 200000 :
                print("Train Finished!")
                if not os.path.isdir("result2_{}/".format(data_augment)) :
                    os.makedirs("result2_{}/".format(data_augment))
                np.savez("result2_{}/curve.npz".format(data_augment), loss=loss_list, acc=acc_list, best_acc = best_acc)
                break
else :
    loss_list = list()
    acc_list = list()
    ...
    for idx, data in enumerate(TrainDataGen):
        ...
        if idx % 1000 == 0:
            ...
            if Overall_accuracy >= best_acc:
                ...
                state = {'model': model.state_dict(),
                         'optimizer': optimizer.state_dict()}
                if not os.path.isdir("ckpt_energy_not_fixed_r_{}/".format(data_augment)):
                    os.makedirs("ckpt_energy_not_fixed_r_{}/".format(data_augment))
                torch.save(state, "ckpt_energy_not_fixed_r_{}/model.pth".format(data_augment))

        if idx == 200000:
            print("Train Finished!")
            if not os.path.isdir("result_energy_not_fixed_r_{}/".format(data_augment)):
                os.makedirs("result_energy_not_fixed_r_{}/".format(data_augment))
            np.savez("result_energy_not_fixed_r_{}/curve.npz".format(data_augment), loss=loss_list, acc=acc_list, best_acc=best_acc)
            break
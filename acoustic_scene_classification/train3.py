...
from utils import MixupGenerator, CutmixGenerator, SpecmixGenerator, SpecaugmentGenerator, RandomMaskingGenerator, EnergymaskingGenerator
...
if data_augment == 'None' :
    ...
    TrainDataGen = SpecmixGenerator(LM_train,
                                  ...
                                  alpha=mixup_alpha,
                                  crop_length=crop_length)()
elif data_augment == 'RandomMasking' :
    TrainDataGen = RandomMaskingGenerator(LM_train,
                                  y_train,
                                  batch_size=batch_size,
                                  alpha=mixup_alpha,
                                  crop_length=crop_length)()
    ...
    for idx, data in enumerate(TrainDataGen):
        ...
        if idx % 1000 == 0:
            ...
            if Overall_accuracy >= best_acc:
                ...
                state = {'model': model.state_dict(),
                         'optimizer': optimizer.state_dict()}
                if not os.path.isdir("ckpt_{}_r0.7/".format(data_augment)):
                    os.makedirs("ckpt_{}_r0.7/".format(data_augment))
                torch.save(state, "ckpt_{}_r0.7/model.pth".format(data_augment))

        if idx == 200000:
            print("Train Finished!")
            if not os.path.isdir("result_{}_r0.7/".format(data_augment)):
                os.makedirs("result_{}_r0.7/".format(data_augment))
            np.savez("result_{}_r0.7/curve.npz".format(data_augment), loss=loss_list, acc=acc_list, best_acc=best_acc)
            break
...
from utils import MixupGenerator, CutmixGenerator, SpecmixGenerator, SpecaugmentGenerator
from focal_loss import Focal_loss, CrossEntropyLoss
from torch.utils.data import DataLoader, Dataset
import torchvision.models as models

ThisPath = 'data/SECL_UMONS/'
num_audio_channels = 1
sr = 44100

TrainFile = ThisPath + 'unilabel_split1_train.csv'
ValFile = ThisPath + 'unilabel_split1_test.csv'
...
wavpaths_train = dev_train_df['0']
scene_labels = list()
y_train_labels = list()
for wavepaths in wavpaths_train :
    label = wavepaths.split('/')[2]
    scene_labels.append(label)
    if label == "chair_movement" :
        y_train_labels.append(0)
    elif label == "cup_drop_off" :
        y_train_labels.append(1)
    elif label == "furniture_drawer" :
        y_train_labels.append(2)
    elif label == "hand_clap" :
        y_train_labels.append(3)
    elif label == "keyboard" :
        y_train_labels.append(4)
    elif label == "knock" :
        y_train_labels.append(5)
    elif label == "phone_ring" :
        y_train_labels.append(6)
    elif label == "radio" :
        y_train_labels.append(7)
    elif label == "speaker" :
        y_train_labels.append(8)
    elif label == "step" :
        y_train_labels.append(9)
    elif label == "whistle" :
        y_train_labels.append(10)
    else :
        assert "error"
ClassNames = np.unique(scene_labels)
NumClasses = len(ClassNames)
y_train = to_categorical(y_train_labels, NumClasses)


dev_val_df = pd.read_csv(ValFile,sep='\t', encoding='ASCII')
wavpaths_val = dev_val_df['0']
scene_labels = list()
y_val_labels = list()
for wavepaths in wavpaths_val :
    label = wavepaths.split('/')[2]
    scene_labels.append(label)
    if label == "chair_movement" :
        ...
        y_val_labels.append(2)
    elif label == "hand_clap" :
        y_val_labels.append(3)
    elif label == "keyboard" :
        y_val_labels.append(4)
    elif label == "knock" :
        y_val_labels.append(5)
    elif label == "phone_ring" :
        y_val_labels.append(6)
    elif label == "radio" :
        y_val_labels.append(7)
    elif label == "speaker" :
        y_val_labels.append(8)
    elif label == "step" :
        y_val_labels.append(9)
    elif label == "whistle" :
        y_val_labels.append(10)
    else :
        assert "error"
ClassNames = np.unique(scene_labels)
NumClasses = len(ClassNames)
